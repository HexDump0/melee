                          ...
                          char *error, size_t error_size)
{
    ...
    rc = disc_load(disc_image, "PlMr.dat", &asset, error, error_size);
    if (rc != DISC_OK) return rc;
    rc = disc_enumerate_public_symbols(&asset, find_mario_root, &root,
                                             error, error_size);
    if (rc != DISC_OK || !root.found) {
        disc_free(&asset);
        attr_error(error, error_size, "ftDataMario symbol not found");
        return DISC_CORRUPT_ARCHIVE;
    }
    ...
    attr_offset += data_base;
    {
        ...
        /* Sandbox physics treats full stick as one acceleration value. The
