...
- `hsd/aobj.c/.h`: literal port of `fobj.c` (parseFloat/parseWait/parsePackInfo,
  CON/LIN/SPL0/SPL/SLP/KEY, `FObjUpdateAnim`, `HSD_FObjInterpretAnim`).
- `hsd/anim.c/.h`: walks the `Pl<Char>AJ.dat` concatenated sub-archives, reads
  ...
  poses the model.
- `hsd/model.c/.h`: keeps raw per-vertex skin inputs (position/normal/group
...
received by `fobj_interpret` but ignored in `hsd_model_pose_channel`.
...
- A node->joint shift search (`ANIM_SHIFT_DEBUG` debug, removed): no constant
...
- `native/hsd/aobj.c/.h`, `native/hsd/anim.c/.h` (new)
- `native/hsd/model.c/.h`, `native/main.c`, `native/CMakeLists.txt`
...
./build/native/melee --list-clips               # 195 Mario clips
...
ASAN_OPTIONS=detect_leaks=0 ./build/native-asan/melee --scripted --frames 600
