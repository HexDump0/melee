...
**Amendment, 2026-09-16 (P-502).** The last clause was the right instruction
...
- the 76 `union CmdUnion` members, plus `gmScriptEventDefault` and the three
  `ColorOverlay_x8_t` groups, are declared over one 32-bit host-order word by
  `native/decomp/shim/decomp_cmd_bits.h`, generated by
  `native/tools/gen_cmd_bits.py`, and read through `CMD_U()` / `CO_X8()`, which
  supply the byte swap.  The **data is not converted** and must not be: a
  script carries relocated host pointers inline (`Command_05`, `Command_07`).
...
`ctest bit_order` is the same test on both toolchains. The decision not to
adopt `scalar_storage_order` for *disc data* is unchanged and is now
unqualified -- the attribute is not used anywhere in the tree.
...
> **Status: accepted with revisions, 2026-09-16.** The audit's architecture
> held; three of its blockers did not survive measurement, and its gate
> ordering is wrong in our favour. Read this box before the body.
>
> | Proposal | Measured |
> |---|---|
...
>
> What remains of W0-W2 is CP932 (not started) and the browser/device matrix.
> Evidence: `native/AI/handoffs/2026-09-16-P-502-*.md`.
