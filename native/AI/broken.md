...
| B-22 | CSS CPU level reads `normar.a.` instead of `Normal`; the VERSUS splash is missing the squiggle across the top and the player name at the bottom | `./build/native/melee --frontend`, walk to the character select | Reproduces headlessly: the CSS name plate under Mario's panel is a solid black box where the text belongs — same shape as your VERSUS report. Tracked as P-700. |
