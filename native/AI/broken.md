...
| B-22 | CSS CPU level reads `normar.a.` instead of `Normal`; the VERSUS splash is missing the squiggle across the top and the player name at the bottom | `./build/native/melee --frontend`, walk to the character select | **Fixed:** the decorations (P-701), the text (P-700: `vsnprintf(buf, -1, ...)` drops the last character on glibc, G-149) and the fighter names (P-703 encoding, G-150; P-704 name-width table read out of bounds, G-154).  Still open: the *player* name plate (the tag under your panel) is an empty black box — **P-702**. |
