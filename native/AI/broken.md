...
| B-22 | CSS CPU level reads `normar.a.` instead of `Normal`; the VERSUS splash is missing the squiggle across the top and the player name at the bottom | `./build/native/melee --frontend`, walk to the character select | The decoration half (marker chain, red VS) is **fixed** by P-701. The text half remains: the CSS level reads `VERY EASE..` instead of `VERY EASY` and the name plate is an empty black box. Tracked as P-700. |
