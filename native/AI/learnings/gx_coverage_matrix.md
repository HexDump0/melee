...
| `GXSetZTexture` REPLACE/ADD/bias, Z8/Z24X8 (4 sites) | EXACT (P-682 + P-691): REPLACE ignores incoming, ADD adds it, bias/0xFFFFFF added in both; colour-writing draws also output the raster colour (screen erase) | dedicated depth program `gx_gl.c:draw_ztex` | decomp `GXTev.c:333` + Dolphin `zbias` (Aurora has a TODO here) | `ctest decomp_efb` passes 2 and 11; title erase base G-138 |
