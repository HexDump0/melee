...
| `src/melee/ft/ftbosslib.c` (`ftBossLib_8015BD24`) | under `PORT_PC`, treat a CPU level of 0 as 1 before `arg3 / arg0` | The level comes from `gm_8017EB64`, a table lookup with no clamp, and can be 0 even though `Player::cpu_level` is documented "1 to 9". PowerPC's `divw` by zero does not trap and leaves the target undefined; x86's `idiv` raises SIGFPE, which killed the Classic Master Hand fight in `ftMh_Wait1_0_Anim`. 1 is the longest interval the table can legally ask for, which is what a level below the minimum means (P-851). |
| `src/melee/cm/camera.c` (`Camera_8002E234`) | under `PORT_PC`, seed `sp10`/`spC`/`sp8` from `game_camera.x368` before the three guarded `Camera_8002E158` calls | Each of the three is written only behind its own `x35C.bits.b*` and then read unconditionally. A boss intro animates only the axes it sets -- Master Hand's entry sets distance and yaw and never the pitch -- so the pitch fed into `game_camera.transform.position` was a leftover: benign on PowerPC, a PIE address or a NaN on 32-bit x86, and `lbvector.c:397` asserted on the first frame of the fight. `x368` is the interpolation's own start value, which is what "not animated" means (P-847, G-205). |
...
command structs were marked with GCC's
...
> **Superseded 2026-09-16 (P-502 gate zero).**  Clang **ignores** that attribute
> in silence, so the WebAssembly build decoded every command wrongly while
> compiling cleanly.  `CMD_BE` is now empty everywhere and the layout is stated
> explicitly in `native/decomp/shim/decomp_cmd_bits.h` (generated by
> `native/tools/gen_cmd_bits.py`), read through `CMD_U()`.  The rows below still
> describe *which* structs needed the treatment and why, which has not changed;
> only the mechanism has.  See G-193.
