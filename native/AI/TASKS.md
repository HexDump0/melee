...
|---|---|---|---|---|---|
| P-663 | S8.0 spike: build Aurora 64-bit on the reference machine (Vulkan/Mesa), verify headless offscreen rendering (`GXCreateFrameBuffer` + screenshot), size Dawn/nod packaging, licenses, build time, and compare "port our tree" vs "rebase on jonrosner/melee-native" | open | — | `native/AI/DECISIONS.md`, `native/AI/logs/`, `build/` (scratch) | ADR-0015. Acceptance: offscreen frame captured, sizing note in `logs/`; do not start S8.1+ before this gate. |
| P-664 | S8.1: 64-bit compiled game target — compat header + generated SDK headers, portability scan (adapt `jonrosner/melee-native`'s `scan_portability.py`), fix LP64 pointer-width/bitfield/global-overlay classes; GC DOL stays byte-identical | open | — | `native/CMakeLists.txt`, `native/decomp/shim/`, `native/decomp/boot/`, `patches/` | ADR-0015. Acceptance: full 64-bit link + headless boot to title; `ctest` (32-bit) stays green. |
| P-665 | S8.2: schema asset materializer — port the MIT `asset_schema.c`/`archive_runtime.cpp`/`archive_bridge.c`/`stage_numeric_layouts.hpp` family by family (fighters → items → stages → menus/effects); converter stays the test oracle | open | — | `native/decomp/assets/`, `native/tests/test_decomp_assets.c` | ADR-0015. Acceptance: every `Pl*`/`ItCo`/`Gr*`/menu archive materializes and loads like today; raw-vs-materialized tests per family. |
| P-666 | S8.3: Aurora GX backend behind `native/decomp/shim/dolphin/gx` with `MELEE_GX_BACKEND=aurora|gles`; present via the existing hook; keep the EGL/GLES path for tests/web | open | — | `native/decomp/gx/`, `native/decomp/shim/dolphin/gx/`, `native/CMakeLists.txt` | ADR-0015. Acceptance: `decomp_frontend`/`decomp_match`/`decomp_hit`/`decomp_icons` pass on Vulkan; screenshot delta logged. |
| P-667 | S8.4: desktop platform adapters — Aurora core/VI/app + PAD bridge; keep our DVD/CISO+ARQ, AX/audio and CARD; decide each API's owner explicitly | open | — | `native/platform/`, `native/audio/` | ADR-0015. Acceptance: interactive Linux run, deterministic tests unchanged. |
| P-668 | S8.5: web path — spike Aurora + `emdawnwebgpu` on wasm32 (browser WebGPU, Emscripten 4.0.10+) behind the GX shim; keep wasm32+GLES green throughout and use it as the fallback until the spike renders a real Melee frame | open | — | `native/`, `native/AI/ROADMAP_DETAILS.md` | ADR-0015/0016, P-502. Acceptance: either an Aurora wasm build rendering the frontend flow, or a written no-go with the blockers and the GLES fallback kept. |
| P-670 | S8.5b: mobile targets — Android (Aurora Vulkan + SDL3) then iOS/tvOS (Metal): window/events/PAD via Aurora, our OS/DVD/ARQ/AX/CARD behind it, disc-image access via SAF/document picker, mobile audio route | open | — | `native/platform/`, `native/`, `native/CMakeLists.txt` | ADR-0016. Acceptance: Android build boots to title on device/emulator with the user's image; no bundled assets. |
| P-669 | S8.6: rebaseline and CI — backend-agnostic render tests (retire/replace `decomp_gx_direct`/`decomp_efb` command-level checks), pin Aurora, licenses, perf numbers, docs; revisit submodule+patches vs fork | open | — | `.github/`, `native/tests/`, `native/AI/` | ADR-0015. Acceptance: dual-backend CI green. |
...
| P-642 | Further match-render optimization: move repeated vertex transform/texgen work toward a GPU/object-space path, or specialize the common GX display-list decoder without hashing/caching dynamic arrays | open | — | `native/decomp/gx/gx_hle.c`, `native/decomp/gx/gx_gl.c` | **Superseded on desktop by ADR-0015 (Aurora S8.3); frozen for web only.** Profile first under a quiet system. Current top CPU costs are `exec_primitive`/`read_vertex`, `read_comp`, `transform_vertex`, and `texgen_coord`. A decoded-display-list cache was tested and reverted because live arrays/state churn made hashing slower than decoding. Preserve direct/dynamic geometry and frame-718 pixel parity. |
...
| P-617 | Indirect-texture shader evaluation + toon ramp evaluation (GX HLE) | open | — | `native/decomp/gx/gx_gl.c` | **Superseded on desktop by ADR-0015 (Aurora S8.3); do not start.** **S4-only.** `GXSetTevIndirect`/`GXSetIndTex*` state is captured per draw (P-612); the GLES fragment path does not yet apply the indirect offsets. Only `lb/lbrefract.c` (stage refraction) uses it, so it cannot be validated until S4 runs a stage. Toon (`GX_TG_SRTG`) currently passes the source value through; stage-only content. |
