...
| P-647 | Dialog/menu text is invisible: the SisLib text engine is still replaced by `platform/font_stub.c` (zero atlases). Compiling the real `hsd_3A76.c`/`hsd_3915.c` plus DOL-extracted atlases crashes in `gx_texture_decode` on the first dialog glyph (`GXInitTexObj` image `0x58d68200`, unmapped; atlas is `0x56dc4200`), so the engine also needs the archive font path (`SdMsgBox`/`SdSlChr` `SIS_*` data) before the swap is safe | open | — | `native/platform/font_stub.c`, `src/sysdolphin/baselib/hsd_3A76.c` (read-only), `native/decomp/gx/gx_gl.c` | Repro: compile `hsd_3A76.c`/`hsd_3915.c` instead of the stub + copy `HSD_SisLib_FontAtlas`/`HSD_DebugFontAtlas` from the user's `sys/main.dol` at DVD init -> segfault in `decode_i4` at the first dialog frame. The atlas bytes/addresses are right (`texture_for` texture 2, 32x32 I4); the texture's `image` comes from the engine's font state, not from the atlas arithmetic. |
