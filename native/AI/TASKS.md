...
|---|---|---|---|---|---|
| P-671 | Renderer parity kickoff: build the GX coverage matrix (every `GX*`/register Melee calls → implemented exactly / approximated / stubbed, with the Aurora reference symbol at the pinned commit), stand up the untracked `aurora-reference` checkout, and file the prioritized gap list | claimed | opencode (deepseek-v4.1-flash), 2026-09-13 | `native/AI/workflows/renderer_parity.md`, `native/decomp/gx/`, `native/AI/learnings/` | ADR-0017. Acceptance: matrix committed under `learnings/` and each red row has a task. Reference commit `749d6ee7a22bdfab78c8ece9047bca5d79aa72ca`. |
| P-672 | Indirect texturing + toon ramps in the GLES renderer (stage refraction, `GXSetTevIndirect`/`GXSetIndTex*`, `GX_TG_SRTG`) | open | — | `native/decomp/gx/gx_gl.c`, `native/decomp/gx/gx_hle.c` | ADR-0017, replaces P-617. Port from Aurora `lib/gx/shader.cpp`/`command_processor.cpp`; regression: render a refraction/ramp fixture and assert reads. |
| P-673 | Channel/lighting/specular parity: verify `GXSetChanCtrl`/`GXSetChanMatColor`/light evaluation against Aurora (`GXLighting.cpp`, `shader_info.cpp`), including attenuation and ambient/diffuse/specular combinations | open | — | `native/decomp/gx/gx_gl.c` | ADR-0017, P-621 follow-up. Regression: lighting fixture matrix + screenshot baseline. |
