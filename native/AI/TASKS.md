...
| P-629 | Sustained match slowdown from viewer frame ~670 (game CPU/frame 10 ms -> 29 ms) together with a broken Link pose; capture counts are identical (draws/verts/lists/prims) and `render=` stays ~2.5 ms, so the cost is in changed per-vertex input (vertex-color decode? NBT?).  Full metrics + perf data in `handoffs/2026-09-12-P-629-match-slowdown.md` | open | — | `native/decomp/gx/gx_hle.c`, `native/decomp/render/viewer_main.c` | Repro: `SDL_VIDEODRIVER=offscreen ./build/native/melee_decomp_viewer --match`; watch `[match]` `game=` jump at frame ~670 and recover ~1080.  `--dump-draws 660` vs `720` is identical.  Next: count colored vertices/format (`decode_color`) and gate the unconditional binormal/tangent transforms on `raw->has_nbt`. |
| P-627 | Intermittent broken fighter pose around KO/respawn (Link's limbs/accessories render at odd angles for a few frames; user "glitches out after respawn").  Captured candidates at viewer frames 610-660 (`/tmp/respawn2.png`) but not reproduced deterministically yet | open | — | `native/decomp/render/viewer_main.c` (capture), `src/melee/ft/` (read-only) | Repro: `./build/native/melee_decomp_viewer --match --frames 650 --shot /tmp/p.bmp`; compare consecutive frames; suspect animation blending (`x8AC_animSkeleton` / `x8A4_animBlendFrames`) or accessory `right` matrices during Fall/Rebirth.  Ask the user for the on-screen frame number next time it glitches. |
