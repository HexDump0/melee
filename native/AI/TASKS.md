...
|---|---|---|---|---|---|
| P-612 | GX HLE polish: indirect/bump/toon, NBT binormal/tangent, dither | open | — | `native/decomp/gx/gx_hle.c`, `gx_gl.c` | **Partial:** >4 TEV stages (limit 8, G-059), texture LOD bias/anisotropy/min-max LOD, scissor (EFB-scaled) and dst-alpha landed, plus the missing `HSD_VIData` render-mode init the scissor exposed (G-061). Remaining: indirect/bump/toon, NBT binormal/tangent, dither (GLES has no `GL_DITHER` control). Not blocking the viewer/S4; take items as they show up in play. |
