...
| P-647 | SisLib fonts: the real `hsd_3A76.c`/`hsd_3915.c` engine compiles and runs (SIS buffers read big-endian via `SIS_U16/S16/S32`; I4 decode fixed); the remaining blocker is the atlas: the DOL is **not in the FST**, so `platform_disc_load_file("sys/main.dol")` fails and the atlases stay zero | open | — | `native/platform/dvd.c`, `native/decomp/fonts.c` | Read the DOL offset from disc header `0x420` and add a raw-DOL special case to `platform_disc_load_file` (section-size math in `decomp/fonts.c` already handles the buffer). Full context and acceptance in `handoffs/2026-09-13-P-647-sislib-fonts.md`. Repro: `MELEE_VIEWER_TRIAGE=1 melee --frontend --input native/tests/no_events.txt --frames 400 --shot /tmp/d.bmp` -> `cannot read 'sys/main.dol' from disc: file not found`; dialog text invisible. |
