...
| P-643 | Random item spawns crash in `HSD_PObjResolveRefs` (`pobj->u.jobj` NULL): an item model joint tree references a joint outside its loaded root; reproduce with the retail frontend match and items on | open | — | `native/decomp/assets/hsd_convert.c` | Repro: `melee_decomp_viewer --frontend --input <flow script>` (default items on) → panic ~2 s into the match. Evidence (gdb): root `0x817F8` tree has 12 joints, PObjDesc references joint `0x81260`, which is not in the tree and does not look like an `HSD_Joint`; the food special-attrs fix (converter v64) is already in. Item models load through `conv_item_model_desc` → `conv_joint`; check the joint walk against a known-good item (Food) and an item archive. |
