...
| P-650 | Title screen missing background animation (sphere grid) and logo fire fill vs Dolphin GALE01 | claimed | Muse Spark, 2026-09-13 | `src/melee/gm/gmtitle.c` (read-only), `native/decomp/gx/`, `native/decomp/assets/hsd_convert.c` | Owner report 2026-09-13 with Dolphin comparison shots. Suspects: title tunnel effects (EFB/indirect P-617?), logo matanim texture scroll, or effect banks. Repro: A-presses at 240/300 to dismiss memcard prompt, `--frames 500 --shot /tmp/title.bmp` (mode=0 title) vs Dolphin. |
| P-649 | Main-menu 1-P preview shows emblem + 1 line then empty instead of the 4-line submenu (Regular/Event/Stadium/Training) | blocked | Muse Spark, 2026-09-13 | `decomp/src/melee/mn/mnmain.c` (read-only), `native/decomp/assets/hsd_convert.c`, `native/decomp/gx/` | Parked with full findings in `handoffs/2026-09-13-P-649-menu-preview.md`. Preview is `tree[14]` joint/texture anim, not glyphs; 5 hidden joints with BRANCH tracks (fterm 450-1050, big text textures incl. 216x144 4-panel) never fire in MAIN loop [0,49]. Repro + ruled-out list in handoff. |
