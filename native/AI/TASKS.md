...
| P-204 | TEV pass: material/RAS stage, colormap/alphamap, alpha test, XLU blend, TEX0+TEX1 | claimed | opencode (deepseek-flash), 2026-09-10 | `native/main.c`, `native/demo_model.c`, `native/demo_model.h` | Increment 1 landed (see `learnings/hsd_tev_materials.md`): `--dump-tev`, GX channel lighting, `HSD_SetupPEMode` blend/alpha-test/Z, `TObjMakeTExp` colormap/alphamap, TEX1. Remaining: `RENDER_SPECULAR` (needs scene `HSD_LObj` lights), lightmap repeat chains, `HSD_TObjTev` active overrides, toon. |
