...
| P-201 | Animation foundation: evaluate HSD `AObj` and drive joint matrices per frame | open | — | `native/demo_model.*`, `native/main.c` | Renderer must stop baking one display list; joint matrices must be recomputed per tick. Acceptance: a looping idle animation plays for `PlMrNr.dat` with no ASan errors. |
| P-202 | Implement `_HSD_mkEnvelopeModelNodeMtx` (`right`) for non-root PObjs | open | — | `native/demo_model.c` | Needed for assets whose DObjs hang off non-skeleton-root joints. Verify with a config asset (e.g. a stage) or a synthetic test. |
| P-203 | TLUT + CI4/CI8 textures | open | — | `native/demo_model.c`, `native/demo_texture.c` | Parse `HSD_TlutDesc`, upload palette, decode indexed formats. Acceptance: character eyes render with texture instead of flat color. |
...
| P-410 | Face/part visibility: expose a manual visibility set, then parse `FtPartsVis` | open | — | `native/demo_model.*`, `native/main.c` | Workaround exists (viewer part isolation). Acceptance: default render shows a neutral face without smearing, or a documented per-part preset. |
