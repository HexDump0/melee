...
| P-648 | Menu BGM (HPS) stops after ~3 s while menu SFX keep playing | blocked | Muse Spark, 2026-09-13 | `native/platform/hps.c`, `native/audio/`, `src/melee/lb/lbaudio_ax.c` (read-only) | Parked with full findings in `handoffs/2026-09-13-P-648-menu-bgm.md`. Proven: menu01 buffers 3 pages then zero reads; page machine deadlocks at (7768,776C)=(1,1), pos frozen at 2; voices end (~767), node torn down. Mixer/DVD/tables innocent. Open: why retail escapes the same circle (initial end/loop vs completion-order?). | Owner report 2026-09-13 from the product frontend. SFX work, so AX/DSP voices are fine; suspect the HPS stream's read-ahead/ARQ loop or the menu track switch. Compare with the boot/title HPS path (S5 verified). |
