...
|---|---|---|---|---|---|
| P-656 | Convert the `ftData->x48_items` per-fighter special `Article` arrays (Ness PK items, Peach, Game & Watch, Link boomerang, …): the six `Article` pointers are relocated but their `ItemAttr`/hurt/states/model/dynamics pointees stay big-endian, so spawned special items read denormal speeds/damage (probe: `PlNs.dat ftDataNess` article attrs +4 = 0.6 BE vs `-6.3e-23` host). Entries must be classified by shape (attr in range, `attr` word0 not a relocation field, `x4_throw_speed_mul`/`x60_scale` sane) because some fighters (Kirby, Yoshi, Pichu) have non-Article pointer data after the real run | open | — | `native/decomp/assets/hsd_convert.c`, `native/tests/test_decomp_assets.c` | Repro: add a `check_ft_data_tables` raw-vs-converted comparison for x48 entry 0's `ItemAttr`; it fails today. Do not walk blindly: a misclassified entry corrupts unrelated data. |
