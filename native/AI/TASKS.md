...
| P-620 | S4: first match — compiled `ft`/`it`/`gr`/`gm` two fighters on a stage, deterministic headless | blocked | opencode (deepseek-v4.1-flash), 2026-09-12 | `native/platform/`, `native/decomp/` | Match runs real frames on Zebes (converter v46, 2 new `src/` patches). Stops at `ftcoll.c:3171` because archive action scripts are BE bitfield words: MWCC packs MSB-first, GCC reads LSB-first (G-082). Next: walk scripts with `opcode = be32(word)>>26` + `ftAction_803C0870[]` advance counts and repack fields; then PAD backend + scripted input. See `handoffs/2026-09-12-P-620-s4-scripts.md`. |
...
| P-622 | S4: repack archive action-command scripts (MWCC MSB-first bitfields → GCC LSB-first) | open | — | `native/decomp/assets/hsd_convert.c` | Walk each `Fighter_WaitAnimData.xC` script with `opcode = be32(word) >> 26` and `ftAction_803C0870[]` advance counts; repack fields per `CmdUnion` (a plain byte swap is wrong, G-082). Stop the `ftcoll.c:3171` assert and continue S4 (`handoffs/2026-09-12-P-620-s4-scripts.md`). |
