...
|---|---|---|---|---|---|
| P-610 | Cross-character GX gaps: Captain Falcon (POBJ_SKIN) renders untextured silver through the compiled path | open | — | `native/decomp/gx/gx_hle.c`, `gx_gl.c` | Repro: `test_decomp_render --model PlCaNr.dat --shot` vs `melee --view --model PlCaNr.dat` (prototype is blue/yellow). Bowser renders magenta-wireframe in the prototype too (parity, not a bug); Kirby/Zelda/Marth/Ganon/Dr Mario/Pikachu/Fox/Luigi match. Suspect the POBJ_SKIN shared-vertex matrix/texture path or vertex colour usage. |
| P-611 | Interactive compiled-path viewer (window + orbit/zoom/model cycle on the S2 GX HLE/GLES3 backend) | open | — | `native/decomp/render/` (new), `native/CMakeLists.txt` | Today the compiled path is headless (`test_decomp_render`); the prototype viewer remains the interactive tool. Needs a 32-bit SDL2 (not installed) or a small EGL + X11/Wayland surface path plus an event loop; `--angle`/`--elevation` on the test already cover headless turntables. |
