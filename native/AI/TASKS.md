...
| P-204 | TEV pass: material/RAS stage, colormap/alphamap, alpha test, XLU blend, TEX0+TEX1 | claimed | opencode (deepseek-flash), 2026-09-10 | `native/main.c`, `native/hsd/model.c`, `native/hsd/model.h`, `native/hsd/parts.c` | Landed: `--dump-tev`, GX channel lighting, `HSD_SetupPEMode` blend/alpha-test/Z, `TObjMakeTExp` colormap/alphamap, TEX0+TEX1, lightmap phases (DIFFUSE/SPECULAR/EXT) with specular accumulation, per-character `model_scaling`. Remaining (see `handoffs/2026-09-10-P-204-tev-materials.md`): real `HSD_LObj` light values, lightmap repeat chains, `HSD_TObjTev` active overrides, toon, `x34_scale.z`. |
