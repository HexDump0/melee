...
|---|---|---|---|---|---|
| P-610 | POBJ_SKIN shared-vertex gaps: Falcon renders untextured silver, Giga Koopa limbs show a noise texture | open | — | `native/decomp/gx/gx_hle.c`, `gx_gl.c` | Repro: `test_decomp_render --model PlCaNr.dat` / `--model PlGkNr.dat` vs the prototype. Bowser's magenta-wireframe prototype look is parity (its archive uses 2 textures). Suspect the shared-vertex two-matrix path (`GXLoadPosMtxImm` slot 0/3 + texture matrix/texgen for the second TObj). All other tested fighters match. |
...
| P-612 | GX HLE polish: texture LOD bias/anisotropy, scissor, dither/dst-alpha, >4 TEV stages, indirect/bump/toon, NBT normals | open | — | `native/decomp/gx/gx_hle.c`, `gx_gl.c` | Everything else the GX HLE records but does not yet apply. Not blocking the viewer/S4; take items as they show up in play. |
