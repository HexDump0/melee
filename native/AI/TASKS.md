...
| P-645 | Results screen names/models are wrong: both panels read "ICE CLIMBERS" and the winner model looks like two characters overlapped, although the match itself used Mario vs DK | open | — | `native/decomp/assets/hsd_convert.c`, `src/melee/ty/toy.c` (read-only) | `gm_801604DC(ckind, mode)` returns `Toy_8030813C(lbl_803B7A00[ckind]) + 4`, i.e. the name string comes from the `TyDatai` trophy tables; a wrong index/table maps every character to the same name. `fn_8017A67C` did receive `kind=CKind_Mario` (gdb), so the standings are right and the name lookup is the suspect. Check the `tyInitModelTbl`/name table conversion against `Toy_8030813C`. |
| P-648 | Menu BGM (HPS) stops after ~3 s while menu SFX keep playing | open | — | `native/platform/hps.c`, `native/audio/`, `src/melee/lb/lbaudio_ax.c` (read-only) | Owner report 2026-09-13 from the product frontend. SFX work, so AX/DSP voices are fine; suspect the HPS stream's read-ahead/ARQ loop or the menu track switch. Compare with the boot/title HPS path (S5 verified). |
...
| P-643 | Random item spawns crash in `HSD_PObjResolveRefs` (`pobj->u.jobj` NULL): an item model joint tree references a joint outside its loaded root; reproduce with the retail frontend match and items on | open | — | `native/decomp/assets/hsd_convert.c` | Repro: `melee_decomp_viewer --frontend --input <flow script>` (default items on) → panic ~2 s into the match (food fix already landed in converter v64; Bob-omb/others remain). Evidence (gdb): model tree with N joints, PObjDesc references a joint outside the tree (`HSD_IDGetData` miss). `--no-items` avoids it for flow tests. |
| P-644 | Match HUD stock icons show Captain Falcon's face for every player (Mario and DK both render Falcon in the stock/percent row) | open | — | `native/decomp/render/` / `native/decomp/assets/hsd_convert.c` | Repro: `melee_decomp_viewer --frontend` → CSS → match; bottom-left icons wrong. Suspect the HUD icon table (`if`/`IfAll` HUD data) or the per-character HUD index table (`ftHudData`/`gm_80160980`), not the fighters themselves (they render correctly). |
| P-630 | `ftData->x1C` part-anim tables are not byte-swapped by the converter: `x0` (part/joint index) and `x2` (parts-list count) read byte-reversed, so `ftAnim_80070F28`/`ftAnim_ApplyPartAnim` loop a count like 3072 over a 12-byte list | open | — | `native/decomp/assets/hsd_convert.c`, `src/melee/ft/ftanim.c` (read-only) | Repro: gdb at match frame 650, Link `ft_data->x1C[0]`: `x0=0x1700` (should be `0x0017`), `x2=3072` (should be 12). Convert the per-entry u16s only after confirming the outer array length (the pointee pointers are relocated; entry 3 looked like a sentinel). Not the P-627 leg cause (x58) but the same converter-omission class. |
| P-642 | Further match-render optimization: move repeated vertex transform/texgen work toward a GPU/object-space path, or specialize the common GX display-list decoder without hashing/caching dynamic arrays | open | — | `native/decomp/gx/gx_hle.c`, `native/decomp/gx/gx_gl.c` | Profile first under a quiet system. Current top CPU costs are `exec_primitive`/`read_vertex`, `read_comp`, `transform_vertex`, and `texgen_coord`. A decoded-display-list cache was tested and reverted because live arrays/state churn made hashing slower than decoding. Preserve direct/dynamic geometry and frame-718 pixel parity. |
...
|---|---|---|---|---|
| P-647 | SisLib text: compile the real font engine with runtime atlases. Compiling the real `hsd_3A76.c`/`hsd_3915.c` plus DOL-extracted atlases crashes in `gx_texture_decode` on the first dialog glyph (`GXInitTexObj` image `0x58d68200`, unmapped; atlas is `0x56dc4200`), so the engine also needs the archive font path (`SdMsgBox`/`SdSlChr` `SIS_*` data) before the swap is safe | done | opencode (deepseek-v4.1-flash) | `native/decomp/fonts.c`, `native/platform/dvd.c`, patches | Repro: compile `hsd_3A76.c`/`hsd_3915.c` instead of the stub + copy `HSD_SisLib_FontAtlas`/`HSD_DebugFontAtlas` from the user's `sys/main.dol` at DVD init -> segfault in `decode_i4` at the first dialog frame. The atlas bytes/addresses are right (`texture_for` texture 2, 32x32 I4); the texture's `image` comes from the engine's font state, not from the atlas arithmetic. | Fixed: the real `hsd_3A76.c`/`hsd_3915.c` compile; `HSD_SisLib_FontAtlas`/`HSD_DebugFontAtlas` are extracted from the user's `sys/main.dol` at DVD init (`decomp/fonts.c`); the SIS text buffers are console big-endian, so the engine reads them through `SIS_U16/S16/S32` accessors (`sislib.h` PORT_PC patch) instead of reordering buffers (the parser dispatches on the first byte). Dialog/title/menu text renders. |
