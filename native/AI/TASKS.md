...
| P-609 | S3: host-endian asset pipeline + DVD/ARQ completion | claimed | opencode (deepseek-flash), 2026-09-12 | `native/platform/{dvd.c,ar.c,os.c,misc.c,disc.c,disc.h}`, `native/decomp/assets/`, `native/decomp/hsd/hsd_scene.c`, `native/CMakeLists.txt` | In progress. Compile the decomp's `dvdfs.c`/`arq.c`, host the DVD/ARAM hardware with a deferred completion pump, promote `hsd_scene.c` into a generic converter with cache + reloc coverage validation, per `learnings/decomp_assets.md` §7. |
