...
| claude (opus-5, frame time) | 2026-09-16 | (released) | **Wound up at the owner's request. All files released, tree clean.** **A match's CPU work is down 49.5%** (66.04e9 -> 33.34e9 instructions): **P-813** `4a9d64be6` the frame texture index was never cleared with the table it indexes (-19.7%), **P-814** `661e14e0a` `GXInitTexObj` dropped the decoded GL texture on every call while HSD re-inits per material bind (-37.1%). Also **P-816** texgen matrices per primitive (-0.77%), **P-818** `GX_HLE_MAX_DRAWS` 1024->4096 and both geometry caps now **report** instead of dropping silently, **P-819** `swap=` printed plus `MELEE_SWAP_INTERVAL`. All byte-identical, ctest 33/33. **Measured and rejected, do not retry:** P-812 texobj scan (wash), P-817 vertex shrink (0.17%), P-820 `-O3` (2.5% but changes rendered output). **P-802 should be re-scoped or closed** on P-817's evidence. **P-821 is CLOSED as not-a-bug (2026-09-17)**: the opponent's costume is *supposed* to be random -- `mncharsel.c:2607` is `HSD_Randi(gm_GetNumCostumesForCKind(...))`, owner confirmed against Dolphin. It only became visible here at P-751, which seeded the RNG from the host clock; before that every playthrough drew the same random costume. **Unfreezing an RNG makes correct-but-varying behaviour look like a regression** -- P-752/P-753 were real bugs from that same change, which is why nobody questioned the premise. Superseded detail: the owner's web-only wrong-costume bug -- my `hsd_convert.c` diagnosis is **retracted and recorded as wrong** (native is fine, the converter is target-independent); the unrun discriminating test is in the row. **P-822** is a real but unverified walk-ordering difference in `hsd_convert.c` found on the way -- **not** P-821's cause. Handoff: `native/AI/handoffs/2026-09-16-frame-time-windup.md`. G-200..G-203. |
