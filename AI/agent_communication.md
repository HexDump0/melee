...
| claude (opus-5, frame time) | 2026-09-16 | `native/decomp/gx/gx_hle.c` | **Round 2, re-claimed.** P-813/P-814 landed (`4a9d64be6`, `661e14e0a`) and took a match's CPU work down 49.5%; owner confirms it "feels much much better" but still drops frames in Giant rounds -- `render=17..25ms` at `draws=909 verts=165129`, while `game` is now only 5.7ms. Ruled out by measurement: **not fill rate** (320x240 to 1920x1440 is flat, 36x the pixels, no change) and **not the texture cache** (2,023 decodes / 600 frames at my heaviest). It is the **CPU vertex pipeline**: ~50% of cycles in `exec_primitive` 15.5%, `texgen_coord` 10.7%, the 144-byte vertex `memcpy` 10.6%, `read_comp` 8.2%. Working **P-816**: hoist the per-vertex texgen and attribute decisions out of the per-vertex loop -- both re-derive state that is constant for the whole primitive run. Byte-identical is the acceptance test. |
