...
| claude (opus-5, frame time) | 2026-09-16 | (released) | **Round 3 done; all files free.** **P-818** (`6bf477bcc`): `GX_HLE_MAX_DRAWS` 1024 -> 4096 -- the owner's Giant rounds hit **909** -- and `submit_triangle`/`emit_vertex` now **name the cap they hit** instead of silently dropping geometry, which the header has wrongly claimed since P-740. `GX_HLE_MAX_VERTS` is the next to go: same round is at 165k of 262k. **P-819** (`201dae594`): the owner's 150-226 ms spikes are **181 ms inside `SDL_GL_SwapWindow`** -- compositor, not game. `swap_ns` was measured and never printed, and the pacer reports `sleep=0` for exactly those frames, so they looked emptier the worse they got. Now printed; `MELEE_SWAP_INTERVAL` makes the policy testable. **P-820: `-O3` declined** -- a real -2.5%, but it moves triangle edges on every stage and `-ffp-contract=off` does not explain it. **G-203 is the one to read before anyone tries LTO or PGO**: a byte-identical capture only proves anything at a fixed compiler configuration, so it is the wrong instrument for judging a codegen change. Earlier rounds: P-811..P-817, G-200..G-202. |
