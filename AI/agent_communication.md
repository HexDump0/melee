...
| opencode (deepseek-v4.1-flash) | 2026-09-17 | `site/**`, `native/AI/reference/branding.md` (untracked), `native/AI/DECISIONS.md` (ADR-0028), root `README.md`/`repo_file_map.md` rows already pending | **Website redesign** to the owner's design comp: fixed left rail nav, hero with the oversized mark, three-steps band, browser/desktop cards, docs cards, FAQ accordion, footer. Static, no build step; preview at `http://localhost:8000`. Copy stays truthful (no browser build, no binary downloads), so "Play in browser" is carded as **Planned** and the primary CTA is build-from-source. Typeface settled as Inter (ADR-0028 + branding.md). Nothing committed yet -- owner asked to hold. |
...
**opencode (Union Alpha), 2026-09-17:** Owner confirmed the previous website session is finished and explicitly handed `site/**` over for a redesign matching the supplied image. Claiming `site/index.html`, `site/styles.css`, and `site/main.js` only. Preserve all other pending work; no commits until the owner explicitly asks.
...
| Commit | What |
|---|---|
| `a7c26881c` | **P-836 mitigated + P-841 filed.** `HSD_GObj_RunProcs` now validates `proc`/`proc->gobj` before dereferencing -- a SIGSEGV *at address 0x1* there is a freed gobj, not a null deref (`p_link` is at +2, `0xFFFFFFFF + 2` wraps to 1). `ft_800852B0` un-aliased: retail reaches **two** tables off `CostumeListsForeachCharacter` and the port named only the first, so `ftData_UnkIntPairs[].data` was never reset (G-217). New `MELEE_WAITANIM_TRACE` in the converter proves all 33 fighter archives convert to exactly the compiled count -- **converter output unchanged, no version bump, no cache clear**. Files: `patches/src/sysdolphin/baselib/gobj.c.patch` (new), `patches/src/melee/ft/ftdata.c.patch`, `native/decomp/assets/hsd_convert.c`. Re-apply patches before your next native build. |
