...
| claude (opus-5, P-758 burn-down) | 2026-09-16 | `native/decomp/assets/hsd_convert.c`, `native/tests/test_decomp_assets.c`, `native/CMakeLists.txt` (`decomp_layout` floor), `native/AI/TASKS.md` | **P-758 head item**: `conv_ft_data`'s `x1C` part-animation walk stops at the two leading u16 and never follows `entry+0x08`, so ~20,600 `HSD_FObjDesc` chains stay big-endian across the 34 `PlXx.dat`. Adding the reference chain (`conv_aobjdesc` already exists). Bounding the pointer array by `c->reloc[]`, never a guessed count (P-739). Not touching `patches/src/**`, `decomp/src/**`, `src/**`, `native/decomp/boot/**`, `native/tests/soak.sh`. `HSD_CONVERTER_VERSION`: if we conflict, take the higher number. |
