...
|---|---|---|---|
| codex (gpt-5, graphics retry) | 2026-09-16 | `native/decomp/gx/gx_gl.c`, `native/decomp/gx/gx_gl.h`, `native/decomp/gx/gx_hle.c`, `native/decomp/gx/gx_hle.h`, `native/tests/test_decomp_render.c`; graphics-specific notes under `native/AI/` only after coordinating | Reopened the owner's dotted/transparent Yoshi's Story/Yoshi's Island foliage, waves, and Bullet Bill issue after P-763 failed the owner's visual retest. Starting from the saved captures and isolating a representative draw/source texture to verify TEV map selection, UV/index decode, and the actual EFB/XFB boundary. I will not touch the active P-758 files (`hsd_convert.c`, `test_decomp_assets.c`, `CMakeLists.txt`, `TASKS.md`) or any patches/decomp source. |
| claude (opus-5, P-758 burn-down) | 2026-09-16 | (released) | **P-758 head item landed, converter v105** -- `266aa9bc6` + `12b2fa4ae`. `conv_ft_part_anim` (`/* DWARF: ftData_x1C */`) follows `ftData->x1C[i]->x8`, the `HSD_AnimJoint*` array `ftAnim_ApplyPartAnim` indexes with `Fighter_x8B0_t.x11`; nothing in the archive records its length, which is why it was never walked. **Coverage 76.87% -> 79.50%** (+5,512 descriptors), `PlXx.dat` 20.2% -> 39.9%. `MELEE_COVERAGE_FLOOR` -> 79.50, `decomp_layout` floor -> 51 (it was already at 50; the ratchet was one behind). ctest 32/32; targeted soak over stages 7/11/31 x all 26 fighters, 78 runs, identical before and after. **`git pull` and rebuild** -- the version key handles `~/.cache/melee/assets`. **Two things worth knowing.** (1) A `c->reloc[slot]` bound does **not** end the `x1C` table: the word after it is `ftData->x20`, whose `x0` is a relocation field too, so the walk read an `HSD_Joint**` array as a fourth descriptor (P-739 shape). `next_pointed_at_after()` is the second bound and both commits now use it. (2) **P-771 is not this chain** -- Corneria fails the same 6 runs with the same `HSD_JObjAddAnim` assertion before and after. `hsd_convert.c` is **free**; I did not touch `patches/src/**`, `decomp/src/**`, `src/**`, `native/decomp/boot/**` or `native/tests/soak.sh`. |
