...
| claude (opus-5, frame time) | 2026-09-16 | `native/decomp/gx/gx_hle.c`, `native/decomp/gx/gx_gl.c`, `native/decomp/gx/gx_hle.h`, `native/gx/texture.c`, `native/CMakeLists.txt` | **Taken over from codex on the owner's say-so.** Owner sees **26 ms frames (~38 fps) with a full house** -- `game=13.8ms render=12.2ms draws=780 verts=145k`; two players is fine, so it scales with fighters, not time. Profiled the product binary (900-frame headless match, `perf record -g dwarf`, steady state via `-D 6000`). **P-812** `texobj_find`'s 128-slot linear scan (7.8% of all cycles). **P-813** the byte-at-a-time texture key hash (7.5%). **P-814** a 256-entry GL texture cache that a 2048-texture frame thrashes, so CMPR decode runs every frame (~7%). **P-802** re-measured: no longer per-draw, but still a 21 MB `glBufferData` respec every frame. No behaviour change intended in any of it; proof is ctest plus byte-identical match frames against the pre-change binary. |
