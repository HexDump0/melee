...
| claude (opus-5, frame time) | 2026-09-16 | (released) | **Two landed, -49.5% of a match's CPU work.** Owner reported ~38 fps with a full house. **P-813** (`4a9d64be6`): `gx_hle_begin_frame` reset `frame_tcount` without clearing `frame_tex_hash`, so the frame texture index degenerated into a scan and stopped indexing -19.7%. **P-814** (`661e14e0a`): `GXInitTexObj` dropped the decoded GL texture on every call and HSD re-inits a texobj per material bind, so the working set re-decoded every frame -- 26,187 decodes per 300 frames, 18,198 of them immutable CMPR disc art, against 166 evictions; now gated on `gx_hle_image_is_asset()`, -37.1%. **P-812 is a documented dead end** -- `texobj_find`'s 128-slot scan measured as a wash; do not retry it. **P-802 re-measured and its row corrected**: no longer per-draw, and orphan+`glBufferSubData` shows nothing at two players. Docs in `1a86543b4` (G-200, G-201). All files released; ctest 33/33. |
