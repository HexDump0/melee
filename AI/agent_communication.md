...
|---|---|---|---|
| claude (opus-5 1M, crash burn-down) | 2026-09-16 | `patches/src/**`, `decomp/src/**` (patched TUs only), `native/decomp/assets/hsd_convert.c`, `native/decomp/boot/**`, `native/tests/**` (except `test_decomp_render.c`), `native/AI/**`, `soak/**`. **Not** `native/gx/texture.c`, **not** `native/decomp/gx/**` -- those stay codex's. | Owner asked for one thing: **make the game stop crashing.** Starting from the finished 5400-frame VS matrix that completed at 19:30 today (`soak/run`): **745 of 754 cells pass, 9 fail**, and every failure has an exact `MELEE_MATCH_*` repro. The nine are: a SIGSEGV in `it_80270E30`, the `lbvector.c:398` **y** assert (P-797, the owner's own crash), the `:397` **x** assert (P-772/P-781 family), one boot-timeout hang, and five `fighter stuck` rows (P-780). Working them in that order, one commit per fix. |
...
| claude (opus-5, frame time) | 2026-09-16 | (released) | **Round 2 done, and it is mostly a negative result worth knowing.** P-816 landed (`5065f21a7`, -0.77%): texgen matrix resolution hoisted out of the per-vertex loop. **P-817 is the useful one** -- shrinking the vertex from 144 to 96 bytes, which P-802 assumes is a big win, measures **0.17%**. With the earlier `read_comp` result in `gx_match_perf.md` (0.01%, reverted), that is three profile-driven micro-optimisations for ~1% between them. **The vertex pipeline is arithmetic, not decisions or bandwidth** (G-202); the only large win left there is moving transform+texgen into a vertex shader. **P-802 should be re-scoped or closed, not implemented.** Ruled out for the owner's Giant-round `render=17..25ms`: fill rate (flat 320x240 to 1920x1440), the texture cache (2,023 decodes/600 frames), and EFB copies (0.02% of cycles). I cannot reproduce it -- my heaviest local scene is 4.3 ms at 703 draws -- so it is waiting on two measurements from the owner. `gx_hle.c` free. |
